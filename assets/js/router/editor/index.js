// Progression Router — editor/DOM layer. Imports the pure modules
// (graph / model / load / persist / planner); owns all rendering, wiring,
// and mutable plan state. The planner never imports from here.
import { createGraph, localStorageAdapter } from "../graph.js";
import { loadJsonl } from "../load.js";
import {
  deriveSkills, normalizeReqs, toState, reqsSummary,
  syncQualEdges as syncStepQualEdges,
} from "../model.js";
...
import { hoursForStep, isAbysmal, loadRates } from "../schedule.js";
  ...
  // One localStorage-backed graph instance (same "osrs-graph:v1" payload the
  // old dal.js used, so existing saves carry over). window.DAL kept for
  // console debugging / back-compat.
  const graph = createGraph(localStorageAdapter());
  window.DAL = graph;
  const dal = () => graph;
  const store = createStore(graph);
  const syncQualEdges = (steps) => syncStepQualEdges(graph, steps);
  ...
  function recompute() {
    ...
    const computed = runPlanner(goalQueue, allSteps, profile, plannerEnv()).path;
  ...
  function loadPlan(plan, idx) {
    ...
    const steps = expandSteps(plan.steps, allSteps);
    currentPath = steps;
  ...
  async function init() {
    ...
    store.plans().forEach((p) => expandSteps(p.steps, allSteps).forEach(extractTags));
